...
test.group('Leaderboard Accuracy', (group) => {
  ...
  test('stats are isolated per session', async ({ assert }) => {
    ...
    // --- Session A ---
    const sessionA = await quizSessionService.startSession(
      user.id,
      quiz.id,
      'STU001',
      'Test School'
    ...
    let statsA = await UserQuizStat.query()
    ...
    // --- Session B (Restart) ---
    const sessionB = await quizSessionService.startSession(
      user.id,
      quiz.id,
      'STU001',
      'Test School'
    ...
    const statsB = await UserQuizStat.query()
      .where('userId', user.id)
      .where('quizId', quiz.id)
      .first()

    assert.equal(
      statsB?.questionsCorrect,
      1,
      'Should only have 1 correct answer (from current session)'
    )
