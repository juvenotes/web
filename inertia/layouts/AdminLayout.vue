...
<template>
  <ThemeProvider default-theme="system" storage-key="juvenotes-theme">
    <div
      class="min-h-screen bg-gradient-to-br from-blue-50 via-white to-slate-100 dark:bg-background dark:bg-none"
    >
      <!-- Top Navigation -->
      <nav
      ...
      <div class="flex">
        ...
        >
          ...
          <nav class="p-4 md:p-6 space-y-2 md:space-y-1 overflow-y-auto h-full">
            <Link
              ...
              :class="{
                'bg-blue-100 text-blue-700 shadow dark:bg-primary/20 dark:text-primary dark:shadow-none':
                  $page.url === item.href,
        ...
        <!-- Main Content -->
        <main
          class="flex-1 p-2 sm:p-4 md:p-8 w-full max-w-full md:max-w-[calc(100vw-15rem)] mx-auto"
        >
          <slot />
        </main>
