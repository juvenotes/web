...
<template>
  ...
  <div class="osce-page-container">
    <div
      class="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8 space-y-6 md:space-y-8"
    >
      <!-- Header -->
      <div
        class="relative p-4 sm:p-6 md:p-8 bg-white rounded-xl md:rounded-2xl border border-gray-100 shadow-md hover:shadow-lg md:hover:shadow-xl transition-shadow duration-300"
      >
        <div
        ...
        <div class="mt-4 sm:mt-6 flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4 md:gap-6">
          <div class="flex items-start gap-3 sm:gap-4 flex-1">
            <div
              class="p-2 sm:p-3 md:p-4 rounded-lg md:rounded-xl bg-[#55A9C4]/10 border border-[#55A9C4]/20 hover:bg-[#55A9C4]/20 transition-colors duration-200"
            >
      ...
      <template v-if="hasPapers">
        <div
          v-for="(yearPapers, year) in papersByYear"
          :key="year"
          ...
          <div class="grid gap-3 sm:gap-4 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
              ...
              <div class="relative space-y-2 sm:space-y-3 md:space-y-4">
                <h3
                  class="text-sm sm:text-base md:text-lg font-semibold text-gray-900 group-hover:text-[#55A9C4] transition-colors duration-300 ease-in-out"
                >
                  {{ paper.title }}
                </h3>
                ...
                <div class="flex flex-wrap items-center gap-2 text-xs sm:text-sm">
                  <span
                    class="px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-md bg-[#55A9C4]/10 text-[#55A9C4] font-medium"
                  >
                    OSCE
                  </span>
                  <span class="text-gray-500"> A few stations </span>
                </div>

                <div
                  class="flex items-center text-xs sm:text-sm text-[#55A9C4] font-medium opacity-0 group-hover:opacity-100 transition-all duration-300 ease-in-out"
                >
      ...
      <div
        v-else
        class="relative p-4 sm:p-6 md:p-8 bg-white rounded-xl md:rounded-2xl border border-gray-100 shadow-md"
      >
        <div class="flex items-start gap-3 sm:gap-4">
          <div
            class="p-2 sm:p-3 rounded-lg md:rounded-xl bg-amber-500/10 border border-amber-500/20"
          >
            <AlertCircle class="h-4 w-4 sm:h-5 sm:w-5 text-amber-500" />
          </div>
          <div class="space-y-0.5 sm:space-y-1">
            <h2 class="text-base sm:text-lg font-semibold text-gray-900">
              No OSCE Papers Available Yet
            </h2>
