...
const handleSubmit = () => {
  // Optionally, prevent submission if password registration is disabled
  // form.post('/register')
}
</script>

<template>
  <AppHead title="Register" description="Start your journey at Juvenotes" />
      ...
      <div class="bg-card rounded-lg shadow p-8">
        ...
        <div class="mb-6">
          ...
          <p class="text-xs text-center text-muted-foreground mt-2">
            We recommend signing up with Google for now.
          </p>
        </div>
        ...
        <div class="relative my-4">
          ...
          <div class="relative flex justify-center text-xs uppercase">
            <span class="bg-background px-2 text-muted-foreground"
              >Or register with email (disabled)</span
            >
          </div>
        </div>

        <!-- Form -->
        <form @submit.prevent="handleSubmit" class="space-y-4 opacity-60 pointer-events-none">
          <!-- Full Name -->
          ...
          <div class="space-y-1.5">
            ...
            <p class="text-xs text-muted-foreground">
              Password registration is currently disabled. Please use Google sign up.
            </p>
          </div>

          <!-- Submit Button -->
          <Button type="submit" class="w-full bg-primary hover:bg-primary/90" :disabled="true">
            Create account
          </Button>
        </form>
      </div>
