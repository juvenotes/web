...
<template>
  <AppHead title="Admin Dashboard" description="Management Dashboard" />
  <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6">
    <div class="flex flex-col sm:flex-row items-center justify-between mb-6 gap-2 sm:gap-0">
      <h1 class="text-2xl font-bold text-foreground dark:text-foreground">Dashboard</h1>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
      <Link
        ...
        :key="item.name + '-card'"
        :href="item.href"
        class="group block rounded-2xl bg-white dark:bg-card shadow-lg hover:shadow-xl border border-slate-100 dark:border-border hover:border-blue-200 dark:hover:border-primary/50 transition-all p-6 text-center cursor-pointer hover:-translate-y-1"
      >
