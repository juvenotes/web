...
function updatePosition() {
  ...
  if (currentStep.value >= 1) { // Steps are 0-indexed, so 1-4 correspond to steps 2-5
...
<template>
  <Teleport to="body">
    ...
    >
      ...
      <div class="relative">
        <!-- Step Number -->
        <div class="absolute -top-2 -right-2 bg-[#55A9C4] text-white w-6 h-6 rounded-full flex items-center justify-center text-sm font-medium">
          {{ currentStep + 1 }}
        </div>
    ...
    <div 
      v-if="isActive"
      class="fixed inset-0 bg-black/30 z-[999]"
      @click="completeTour"
    ></div>
  </Teleport>
</template>
