...
<template>
  <Dialog :open="open" @update:open="$emit('update:open', $event)">
    <DialogContent class="sm:max-w-[425px] max-h-[90vh] overflow-y-auto">
      <DialogHeader>
        <DialogTitle>Create Question of the Day</DialogTitle>
      </DialogHeader>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="space-y-2">
          ...
          <p v-else class="text-sm text-muted-foreground">
            Select the date when this will run. You can add questions after.
          </p>
        </div>
        ...
        <div class="space-y-2">
          ...
          <p v-else class="text-sm text-muted-foreground">
            Enter a descriptive title like "Mental Health Day" or auto-generate from date
          </p>
        </div>
        ...
        <div class="space-y-2">
          ...
          <p v-else class="text-sm text-muted-foreground">
            By default, leave as scheduled unless you want it to run that instant
          </p>
        </div>
