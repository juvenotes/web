...
<template>
  <Dialog :open="open" @update:open="$emit('update:open', $event)">
    <DialogContent class="w-[95vw] sm:max-w-[400px] max-h-[60vh] overflow-y-auto">
      <DialogHeader class="bg-background/95 backdrop-blur-sm z-20 p-4 border-b max-w-screen-lg mx-auto">
        <DialogTitle class="text-lg sm:text-xl text-destructive">Delete Event</DialogTitle>
      </DialogHeader>
      <div class="p-3 sm:p-6">
        <p class="mb-4 text-base">Are you sure you want to delete <b>{{ props.event.title }}</b>? This action cannot be undone.</p>
