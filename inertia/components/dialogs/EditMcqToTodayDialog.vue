...
import ExplanationEditor from '~/components/ExplanationEditor.vue'
...
const handleSubmit = () => {
  ...
  form.put(`/manage/today/${props.today.slug}/questions/${props.question.slug}/mcq`, {
    preserveScroll: true,
    onSuccess: () => {
...
<template>
  <Dialog :open="open" @update:open="$emit('update:open', $event)">
    <DialogContent class="w-[95vw] sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
      ...
      >
        <DialogTitle class="text-lg sm:text-xl">Edit MCQ in {{ today.title }}</DialogTitle>
      </DialogHeader>
